...
parser.add_argument("-r","--removals",required=False,help="the removing set")
...
parser.add_argument("-e","--explore",required=False,action="store_true",help="only solve k-shrink problem and do not do the statistical selection for k")
parser.add_argumetn("-l","--log",require=False,help="keep a detailed log on the removing sets and the optimal diameters for all values 1 to k")
parser.add_argument("-k","--k",required=False,help="the maximum size of the removing set. Treeshrink will solve the k-shrink optimization problem for this k. Default: 2*sqrt(n), where n is the size of the full tree")
